...
with torch.no_grad():
    for index, (rgb, path) in enumerate(tqdm(testloader)):
        ...
        # -------- FIXED SAVE PATH LOGIC --------
        if os.path.splitext(os.path.basename(output))[1] == '':  # output is a directory
            img_path = path[0]
            if os.path.isdir(input):
                sub_path = os.path.relpath(img_path, start=input)
            elif os.path.isfile(input):
                sub_path = os.path.basename(img_path)
            else:
                sub_path = os.path.basename(img_path)
        ...
        if not (os.path.isfile(filename_out)):
            try:
                ...
                print(f"Saved: {filename_out}")  # <-- Added confirmation print
            except:
                import traceback
                traceback.print_exc()
                pass
